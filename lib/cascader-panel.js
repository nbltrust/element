  ...
  methods: {
    ...
    renderPrefix: function renderPrefix(h) {
      ...
      var checkStrictly = config.checkStrictly,
          ...
          onlyLeafMulti = config.onlyLeafMulti;
    ...
    renderCheckbox: function renderCheckbox(h) {
      ...
      if (config.checkStrictly || config.onlyLeafMulti) {
    ...
    renderNumberIcon: function renderNumberIcon(h) {
      ...
      var count = node.calculateCheckedLeaf(node);
      return count > 0 ? h(
        'div',
  ...
  render: function render(h) {
    ...
    if (isLeaf && !isDisabled && !checkStrictly && multiple && onlyLeafMulti) {
      events.on.click = function (e) {
        _this3.handleMultiCheckChange(!node.checked);
      };
    }
    ...
    return h(
      ...
      [this.renderPrefix(h), this.renderContent(h), this.renderNumber(h), this.renderPostfix(h)]
    );
  }
});
...
var node_Node = function () {
  ...
  Node.prototype.calculateCheckedLeaf = function calculateCheckedLeaf(nodes) {
  ...
  Node.prototype.numbAndPath = function numbAndPath(nodes, resData) {
    var this_ = this;
  ...
  Node.prototype.isSameNode = function isSameNode(checkedValue) {
    ...
    return (this.config.multiple || this.config.onlyLeafMulti) && Array.isArray(checkedValue) ? checkedValue.some(function (val) {
...
/* harmony default export */ var cascader_panelvue_type_script_lang_js_ = ({
  ...
  mounted: function mounted() {
    ...
    if (this.config.onlyLeafMulti) {
      ...
      var allN = this.getFlattedNodes(false);
      if (!Object(util_["isEmpty"])(allN)) {
  ...
  methods: {
    ...
    */
    ...
    getCheckedNodes: function getCheckedNodes(leafOnly) {
      ...
      if (multiple) {
        var nodes = this.getFlattedNodes(leafOnly); // leafOnly = true 时，拿到的全是叶子 node 信息(包含不同路径下的叶子节点)
    ...
    clearNotInCurrentPathNodes: function clearNotInCurrentPathNodes(node) {
      var _this6 = this;

      if (node.checked === false) return;
      var path = node.parent ? node.parent.path : node.path;
      var allNodes = this.getFlattedNodes(false);
      allNodes.filter(function (n) {
        return n.value !== node.value;
      }).forEach(function (compareNode) {
        if (compareNode.value !== node.value) {
          if (!compareNode.hasChildren) {
            var comparePath = compareNode.parent ? compareNode.parent.path : compareNode.path;
            var isSamePath = Object(util_["isEqual"])(comparePath, path);
      ...
      this.$nextTick(function () {
        _this6.calculateMultiCheckedValue(); // update panel.checkedValue []
        _this6.syncActivePath();
        _this6.calculateCheckedNodePaths();
      });
    }
  }
});
